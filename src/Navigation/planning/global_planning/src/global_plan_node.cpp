...
private:
    ...
    autoware_msgs::Lane result_path;
    ...
    void convert_result_to_path(autoware_msgs::Lane* result_path);

    autoware_msgs::Lane smooth_path(autoware_msgs::Lane path);

    void smooth(autoware_msgs::Lane& path, int start_index, int length, int step, double weight_data, double weight_smooth, double tolerance);
...
void global_plan::init()
{
    ...
    pub_path = nh.advertise<autoware_msgs::Lane>("global_path", 1);
...
void global_plan::endPose_cb(const geometry_msgs::PoseStampedConstPtr& msg)
{
    ...
    result_path.header.frame_id = "map";
    convert_result_to_path(&result_path);
    autoware_msgs::Lane res_path = smooth_path(result_path);
...
void global_plan::convert_result_to_path(autoware_msgs::Lane* result_path)
{
    int size = result.size();
    autoware_msgs::Waypoint waypoint_start;
    ...
    for (int i = 1; i < size - 1; i++) {
        autoware_msgs::Waypoint pre = result_path->waypoints[result_path->waypoints.size() - 1];
        int id = result[i];
        nav_msgs::Path next;
...
autoware_msgs::Lane global_plan::smooth_path(autoware_msgs::Lane path)
{
    autoware_msgs::Lane path_in;
    ...
    std::vector<autoware_msgs::Waypoint>::iterator it_front, it_end;
    double th_length_front = 8.0, th_length_end = 8.0;
    double sum_length_front = 0, sum_length_end = 0;
    std::vector<autoware_msgs::Waypoint> in_front, in_end; // insert waypoints
    ...
    if (sum_length_front >= th_length_front) {
        std::vector<autoware_msgs::Waypoint> lane_front;
        ...
        autoware_msgs::Waypoint s = in_front[front_index];
    ...
    if (sum_length_end >= th_length_end) {
        std::vector<autoware_msgs::Waypoint> lane_end;
        ...
        autoware_msgs::Waypoint e = in_end[in_end.size() - 1];
    ...
    autoware_msgs::Lane smoothPath_out;
    ...
    for (size_t i = 0; i < smoothPath_out.waypoints.size() - 1; i++) {
        autoware_msgs::Waypoint p_c = smoothPath_out.waypoints[i];
        autoware_msgs::Waypoint p_n = smoothPath_out.waypoints[i + 1];
...
void global_plan::smooth(autoware_msgs::Lane& path, int start_index, int length, int step, double weight_data, double weight_smooth, double tolerance)
{
    double change = tolerance;
    int Iterations = 0;
