 ...
 */
/**
 * @file mpc_follower_core.cpp
 * @brief 此节点主要是接收/mpc_waypoints_converter节点发布的路径信息后，经过计算后发布横纵向目标控制指令
 * 该节点包含：
 *      路径过滤器：应用路径平滑过滤器以减少航点噪声。目前，使用移动平均滤波器。
 *      mpc求解器：解决MPC的优化问题。目前，使用最小二乘法，因此不支持约束。
 *      车辆模型：生成MPC矩阵。目前，仅提供自行车运动学模型。
 *      低通滤波器：二阶巴特沃斯滤波器。
 * 输入：
 *      车辆当前的位姿信息 /current_pose
 *      车辆的状态信息：当前车速、当前转向角 /vehicle_status
 *      接收/mpc_waypoints_converter节点发布的局部路径信息/mpc_waypoints
 * 输出：
 *      发布目标速度指令：目标纵向速度，目标偏航角速度(二选一)
 *      发布目标控制指令：目标转向角，目标纵向速度，目标加速度（二选一）
 *      debug调试相关的话题
 * @date 2020-3-27
 */
...
{
  pnh_.param("is_lastpoints_", is_lastpoints_, bool(false));// 是否到达最后路径点
  pnh_.param("show_debug_info", show_debug_info_, bool(false));// 显示调试信息
  pnh_.param("ctrl_period", ctrl_period_, double(0.03));// 控制周期
  pnh_.param("enable_path_smoothing", enable_path_smoothing_, bool(true));// 路径光滑处理，当使用路径重采样以减少重采样噪声时，应为true
  pnh_.param("enable_yaw_recalculation", enable_yaw_recalculation_, bool(false));// 重新采样后重新计算偏航角。如果接收到的航路点中的偏航有噪声，则设置为true
  pnh_.param("path_filter_moving_ave_num", path_filter_moving_ave_num_, int(35));
  pnh_.param("path_smoothing_times", path_smoothing_times_, int(1));// 应用路径平滑滤波器的次数
  pnh_.param("curvature_smoothing_num", curvature_smoothing_num_, int(35));// 从三个点计算曲率：p（i-num），p（i），p（i + num）。较大的数字会使噪声值较小
  pnh_.param("traj_resample_dist", traj_resample_dist_, double(0.1)); // 重采样轨迹的长度[m]
  pnh_.param("admisible_position_error", admisible_position_error_, double(5.0));// 允许的位置误差,大于该值车辆停止
  pnh_.param("admisible_yaw_error_deg", admisible_yaw_error_deg_, double(90.0));// 允许的角度误差,大于该值车辆停止
  pnh_.param("output_interface", output_interface_, std::string("all"));// 输出车辆控制指令，ctrl_raw或twist_raw，或者两者同时输出

  // mpc参数
  pnh_.param("mpc_prediction_horizon", mpc_param_.prediction_horizon, int(70));// 预测时域
  pnh_.param("mpc_prediction_sampling_time", mpc_param_.prediction_sampling_time, double(0.1));// 周期
  pnh_.param("mpc_weight_lat_error", mpc_param_.weight_lat_error, double(1.0));// MPC横向误差的权重
  pnh_.param("mpc_weight_heading_error", mpc_param_.weight_heading_error, double(0.0));// MPC航向误差的权重
  pnh_.param("mpc_weight_heading_error_squared_vel_coeff", mpc_param_.weight_heading_error_squared_vel_coeff, double(0.3));
  pnh_.param("mpc_weight_steering_input", mpc_param_.weight_steering_input, double(1.0));// MPC的转向误差权重
  pnh_.param("mpc_weight_steering_input_squared_vel_coeff", mpc_param_.weight_steering_input_squared_vel_coeff, double(0.25));// MPC转向误差的重量速度系数（实际转向-参考转向）
  ...
  pnh_.param("delay_compensation_time", mpc_param_.delay_compensation_time, double(0.0));// MPC的时间延迟补偿
  pnh_.param("steer_lim_deg", steer_lim_deg_, double(35.0));// 前轮最大转角
  pnh_.param("vehicle_model_wheelbase", wheelbase_, double(2.9));// 底盘轴长（后轮中心到前轮中心的距离）

  // 车辆模型有三种：自行车模型（kinematics）、运动学自行车模型（kinematics_no_delay）和动力学自行车模型（dynamics）
  ...
  /* set up ros system */
  timer_control_ = nh_.createTimer(ros::Duration(ctrl_period_), &MPCFollower::timerCallback, this);// 创建定时器，roscpp的定时器会计划在某一速率下执行一次回调操作，在订阅或服务中的回调函数队列机制中使用
  ...
  pub_twist_cmd_ = nh_.advertise<geometry_msgs::TwistStamped>(out_twist, 1);//发布目标速度指令：目标纵向速度，目标偏航角速度
  pub_steer_vel_ctrl_cmd_ = nh_.advertise<autoware_msgs::ControlCommandStamped>(out_vehicle_cmd, 1);// 发布目标控制指令：目标转向角，目标纵向速度，目标加速度
  sub_ref_path_ = nh_.subscribe(in_waypoints, 1, &MPCFollower::callbackRefPath, this);// 接收/mpc_waypoints_converter节点发布的局部路径信息
  sub_pose_ = nh_.subscribe(in_selfpose, 1, &MPCFollower::callbackPose, this);// 获取车辆当前位姿
  sub_vehicle_status_ = nh_.subscribe(in_vehicle_status, 1, &MPCFollower::callbackVehicleStatus, this);// 获取车辆的状态信息：当前车速、当前转向角
...
//###################################################
//   接收/mpc_waypoints_converter节点发布的局部路径信息
//###################################################
void MPCFollower::callbackRefPath(const autoware_msgs::Lane::ConstPtr &msg)
{
  current_waypoints_ = *msg;
  ROS_INFO("[MPC] path callback: received path size = %lu", current_waypoints_.waypoints.size());
  ...
  // 设置路径的最后一个路径点的目标速度为0
  const double mpc_predict_time_length = (mpc_param_.prediction_horizon + 1) * mpc_param_.prediction_sampling_time + mpc_param_.delay_compensation_time + ctrl_period_;// 计算mpc的预测时长
