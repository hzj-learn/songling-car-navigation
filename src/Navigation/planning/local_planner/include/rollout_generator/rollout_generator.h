...
private:
    ...
    ros::Publisher pub_centralPathSectionRviz;
    ...
    void visualLaneInRviz(const autoware_msgs::Lane& path, ros::Publisher pub);
    ...
    void getGlobalPlannerPath_cb(const autoware_msgs::LaneConstPtr& msg);

    void getNavGlobalPlannerPath_cb(const nav_msgs::PathConstPtr& msg);

    void msgLane2LocalLane(const autoware_msgs::Lane& trajectory, std::vector<UtilityNS::WayPoint>& path);
