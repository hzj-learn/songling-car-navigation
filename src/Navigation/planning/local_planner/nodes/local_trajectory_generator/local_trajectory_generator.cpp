...
{
    sub_currentPose = nh.subscribe("current_pose", 1, &LocalTrajectoryGenerator::getCurrentPose_cb, this); // 获取机器人当前位姿
    sub_detectedObjects = nh.subscribe("/detection/lidar_detector/objects", 1, &LocalTrajectoryGenerator::getDetectedObjects_cb, this); // 获取障碍物数组
    sub_localRollouts = nh.subscribe("local_rollouts", 1, &LocalTrajectoryGenerator::getRolloutPaths_cb, this); // 获取获选候选路径rollouts
    sub_centralPath = nh.subscribe("centralPathSection", 1, &LocalTrajectoryGenerator::getCentralPathSection_cb, this);// 获取截取的中心路径
    ...
    pub_LocalWeightedTrajectory = nh.advertise<autoware_msgs::LaneArray>("local_trajectories", 1);
    pub_TrajectoryCost = nh.advertise<autoware_msgs::Lane>("local_trajectory_cost", 1);
    pub_testLane = nh.advertise<visualization_msgs::Marker>("test_lane", 1);// 中心路径可视化
    pub_test_points = nh.advertise<visualization_msgs::MarkerArray>("test_points", 1);
    // pub_best_trajectories = nh.advertise<autoware_msgs::Lane>("best_local_trajectories", 1);// 发布最优路径

    pub_base_trajectories = nh.advertise<autoware_msgs::Lane>("base_waypoints", 1);// 发布最优路径
    pub_final_trajectories = nh.advertise<autoware_msgs::Lane>("final_waypoints", 1);// 发布最优路径
    pub_closest_index = nh.advertise<std_msgs::Int32>("closest_waypoint",1);//最近路径点序号

    nh_private.param<double>("horizonDistance", plan_params.horizonDistance, 200);
    nh_private.param<int>("rollOutNumber", plan_params.rollOutNumber, 6);// 候选路径数量
    nh_private.param<double>("rollOutDensity", plan_params.rollOutDensity, 0.1);// 候选路径之间距离间隔
    nh_private.param<double>("horizontalSafetyDistancel", plan_params.horizontalSafetyDistancel, 1);// 水平安全距离
    nh_private.param<double>("verticalSafetyDistance", plan_params.verticalSafetyDistance, 1);// 垂直安全距离
    ...
    nh_private.param<double>("width", car_info.width, 0.5);// 车身宽度
    nh_private.param<double>("length", car_info.length, 0.68);// 车身长度
    nh_private.param<double>("wheel_base", car_info.wheel_base, 0.48);// 前后轮之间轴距
}
LocalTrajectoryGenerator::~LocalTrajectoryGenerator()
{
}
 ...
 * @description: 获取障碍物数组
 */
void LocalTrajectoryGenerator::getDetectedObjects_cb(const autoware_msgs::DetectedObjectArrayConstPtr& msg)
{
 ...
 */
void LocalTrajectoryGenerator::set_detectObj(UtilityNS::DetectedObject& out_obj, const autoware_msgs::DetectedObjectArrayConstPtr& msg, const double& i)
{
 ...
 */
void LocalTrajectoryGenerator::getRolloutPaths_cb(const autoware_msgs::LaneArrayConstPtr& msg)
{
    if (msg->lanes.size() > 0) {
 ...
 * @return: 
 */
void LocalTrajectoryGenerator::getCentralPathSection_cb(const autoware_msgs::LaneConstPtr& msg)
{
...
void LocalTrajectoryGenerator::run()
{
    ...
    while (ros::ok()) {
        ...
        if (currentPose_flag && centralPathSection.size() > 0 && velodyne_map_tf_ok) {
            ...
            best_index = floor(0.6 * best_trajectory.index + 0.2 * pre_index + 0.2 * pre_pre_index);
            //best_index = best_trajectory.index;
            autoware_msgs::Lane best_local_lane;
            ...
            autoware_msgs::LaneArray local_trajectories_msg;
            autoware_msgs::Lane best_local_trajectory;
            std_msgs::Int32 closest_waypoint;
	    
            closest_waypoint.data = 1;
            pub_closest_index.publish(closest_waypoint);
            ...
            for (int m = 0; m < generated_rollouts.size(); m++) {
                ...
                // std::cout<< "last choose best_index=?????:"<< best_index << std::endl;
                if (m == best_index) {
                    ...
                    pub_base_trajectories.publish(best_local_trajectory);
                    pub_final_trajectories.publish(best_local_trajectory);
		    //ROS_INFO("Push best trajectory!!!!!");
                }
 ...
 * @description: 计算轨迹代价函数（静态）
     param:候选路径、中心路径、当前位姿、算法参数、车子参数、障碍物数组
 */
    ...
    const std::vector<UtilityNS::DetectedObject>& obj_list)
{
    ...
    //std::cout<< "params.rollOutNumber:"<< params.rollOutNumber << "  car_relativeInfo.perp_distance : "<<car_relativeInfo.perp_distance<<"  params.rollOutDensity:"<<params.rollOutDensity<< std::endl;
   // std::cout<< "current_index=:"<< currIndex << std::endl;
    ...
    //std::cout <<"pre_best_index:"<<pre_best_index<<std::endl;
    ...
    const UtilityNS::CAR_BASIC_INFO& carInfo)
{
    /*
                                    ...
                                    |      ____________  __   | 
                                          ...
                                          |      |     |
                                          ||----------|| --
                                          |____________| __ critical_long_back_distance

    */

    double critical_lateral_distance = carInfo.width / 2.0 + params.horizontalSafetyDistancel;// 车子横向距离
    double critical_long_front_distance = carInfo.wheel_base / 2.0 + carInfo.length / 2.0 + params.verticalSafetyDistance;// 车子纵向距离
    double critical_long_back_distance = carInfo.length / 2.0 - carInfo.wheel_base / 2.0 + params.verticalSafetyDistance;// 车子后端纵向距离
    ...
    if (rollOuts.size() > 0 && rollOuts[0].size() > 0) {
        ...
        UtilityNS::getRelativeInfo(centerPath, currPose, car_rela_info);// 计算当前位置到中心路径的距离
        UtilityNS::visualLaneInRviz(centerPath, pub_testLane);// 中心路径可视化
        for (int i = 0; i < rollOuts.size(); i++) {
            for (int k = 0; k < contourPoints.size(); k++) {
                UtilityNS::RelativeInfo contour_rela_info;

                UtilityNS::getRelativeInfo(centerPath, contourPoints[k], contour_rela_info);// 计算当前候选路径到中心路径的距离
