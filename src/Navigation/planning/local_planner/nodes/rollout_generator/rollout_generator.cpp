...
{
    nh_private.param<double>("samplingTipMargin", PlanningParams.carTipMargin, 4);// cartip部分从车辆中心点到水平采样的起点，这部分的长度决定了车辆切换不同轨迹的平滑程度。
    nh_private.param<double>("samplingOutMargin", PlanningParams.rollInMargin, 16);// rollin部分从水平采样的起点到平行采样的起点，这部分的长度和车辆速度密切相关，车辆速度越快，rollin部分应越长，使得轨迹更加平滑。
    nh_private.param<double>("samplingSpeedFactor", PlanningParams.rollInSpeedFactor, 0.25);// rollout部分从平行采样的起点到最大规划距离，这部分里每一条rollout都是平行的，相隔距离由rollout_density来确定。
    nh_private.param<bool>("enableHeadingSmoothing", PlanningParams.enableHeadingSmoothing, false);

    nh_private.param<double>("horizonDistance", PlanningParams.horizonDistance, 200); // 无关
    nh_private.param<double>("pathDensity", PlanningParams.pathDensity, 0.5);// 路径点之间的间隔
    nh_private.param<bool>("enableLaneChange", PlanningParams.enableLaneChange, false);// 无关
    nh_private.param<double>("maxLocalPlanDistance", PlanningParams.microPlanDistance, 80);// 最大规划距离

    nh_private.param<double>("maxVelocity", PlanningParams.maxSpeed, 6.0);// 无关
    nh_private.param<double>("minVelocity", PlanningParams.minSpeed, 0.1);// 无关

    nh_private.param<double>("rollOutDensity", PlanningParams.rollOutDensity, 0.5);// 平行候选路径之间的间隔
    nh_private.param<int>("rollOutsNumber", PlanningParams.rollOutNumber, 6);// 候选路径数量
    ...
    pub_localTrajectories = nh.advertise<autoware_msgs::LaneArray>("local_rollouts", 1);// 起于当前位置，止于最大规划距离的平滑候选路径
    pub_localTrajectoriesRviz = nh.advertise<visualization_msgs::MarkerArray>("local_rollouts_rviz", 1); // 候选路径的可视化
    pub_centralPathSection = nh.advertise<autoware_msgs::Lane>("centralPathSection", 1);// 中心路径可视化
    pub_centralPathSectionRviz = nh.advertise<visualization_msgs::MarkerArray>("centralPathSectionRviz", 1);
    pub_testLane = nh.advertise<visualization_msgs::Marker>("test_lane", 1); // 截取路径可视化
...
void RolloutGenerator::run()
{
    ...
    while (ros::ok()) {
        ...
        if (currentPose_flag && globalPaths.size() > 0) {
            ...
            UtilityNS::visualLaneInRviz(centralTrajectorySmoothed, pub_testLane);
            ...
            for (size_t k = 0; k < rollOuts.size(); k++) {
                for (size_t m = 0; m < rollOuts[k].size(); m++) {
                    ...
                    for (int h = 0; h < rollOuts[k][m].size(); h++) {
                        ...
                        //std::cout << "四元数：xyzw    "<<wp.pose.pose.orientation.x<<","<<wp.pose.pose.orientation.y<<","<<wp.pose.pose.orientation.z<<","<<wp.pose.pose.orientation.w<<std::endl;
...
/***************************************************************************************/
            ...
            // 截取规划路径
/***************************************************************************************/
            autoware_msgs::Lane central_path_section;
    ...
    std::vector<UtilityNS::WayPoint>& sampledPoints_debug)
{
    ...
    if (microPlanDistance <= 0)
        return;
    rollOutsPaths.clear();
    sampledPoints_debug.clear(); // 仅仅为了可视化
 ...
 * @return: 
 */
void RolloutGenerator::getGlobalPlannerPath_cb(const autoware_msgs::LaneConstPtr& msg)
{
...
void RolloutGenerator::getGlobalPlannerPath_cb(const autoware_msgs::LaneConstPtr& msg)
{
    if (msg->waypoints.size() > 0) {
        ...
        for (size_t i = 0; i < msg->waypoints.size(); i++) {
            ...
            pp.pos.x = msg->waypoints[i].pose.pose.position.x;
            pp.pos.y = msg->waypoints[i].pose.pose.position.y;
            pp.pos.z = msg->waypoints[i].pose.pose.position.z;
            pp.pos.yaw = tf::getYaw(msg->waypoints[i].pose.pose.orientation);
...
void RolloutGenerator::msgLane2LocalLane(const autoware_msgs::Lane& msg_path, std::vector<UtilityNS::WayPoint>& path)
{
    path.clear();
...
void RolloutGenerator::visualLaneInRviz(const autoware_msgs::Lane& path, ros::Publisher pub)
{
